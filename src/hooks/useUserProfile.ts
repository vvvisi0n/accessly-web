...
export default function useUserProfile() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("accessly-user-profiles");
    if (stored) setProfiles(JSON.parse(stored));
    const current = localStorage.getItem("accessly-current-profile");
    if (current) {
  ...
  // Persist on change
  useEffect(() => {
    localStorage.setItem("accessly-user-profiles", JSON.stringify(profiles));
    localStorage.setItem("accessly-current-profile", JSON.stringify(profile));
  }, [profile, profiles]);
